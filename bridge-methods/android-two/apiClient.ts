...
apiClient.interceptors.request.use(
  async (config: InternalAxiosRequestConfig) => {
    ...
    const publicParams = await window.PG_Finance.getSession(url);
    const decryParam = JSON.parse(await window.PG_Finance.decodeData(publicParams) || "{}");
    config.params = { ...newParams, ...decryParam };
    ...
    if (regImgUrl.test((config.url || "").split("?")[0])) {
      ...
      const aesDecodeData = await window.PG_Finance.encodeData(JSON.stringify(paramDate || {}));
      switch (reqMethod) {
        case "get":
          config.data = qs.stringify({ ["dictate"]: aesDecodeData });
          break;
        case "post":
          config.data = qs.stringify({ ["uncertainty"]: aesDecodeData });
          break;
        default:
          config.data = qs.stringify({ ["monkey"]: aesDecodeData });
      }
      config.method = "post";
      config.url = forwordPreifx + config.url;
      return config;
...
apiClient.interceptors.response.use(
  async (response: AxiosResponse) => {
    const deData = await window.PG_Finance.decodeData(response.data["course"] || "");
    if (typeof deData === "string" && isJSONString(deData)) {
      response.data["course"] = JSON.parse(deData);
    } else {
      if (typeof deData === "string" && (deData === '""' || deData === '""""')) {
        response.data["course"] = "";
      } else {
        response.data["course"] = deData;
      }
    }
